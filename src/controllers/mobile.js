...
export default function mobileinit() {
    ...
    //双指缩放    
    let lastDistance = 0; // 上一次双指间距离
    let lastX1 = 0; // 上一次第一个触点的水平坐标
    let lastY1 = 0; // 上一次第一个触点的垂直坐标
    let lastX2 = 0; // 上一次第二个触点的水平坐标
    let lastY2 = 0; // 上一次第二个触点的垂直坐标
    var p = 0;
    var lastYcenter;
    $(document).on("touchstart", "#luckysheet-grid-window-1", function (event) {
        ...
        if (touches.length == 1) {
            ...
            lastDistance = distance; // 保存上一次双指间距离
            // 保存上一次触点坐标
            lastX1 = x1;
            lastY1 = y1;
            lastX2 = x2;
    ...
    $(document).on("touchmove", "#luckysheet-grid-window-1", function (event) {
        let touches = event.originalEvent.targetTouches
        if (touches.length == 1) {
            ...
            var distance = Math.sqrt(Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2));
            // 计算缩放比例
            scale = (lastScale * distance) / lastDistance;
            zoomChange(scale);
